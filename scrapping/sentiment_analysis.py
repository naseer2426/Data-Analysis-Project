...
def find_max(my_dictionary):
    ...
    converter = {"positive": 1, "neutral": 0, "negative": -1}
    return converter[max_key]


sent = parse_data("caption_sentiment_analysis.csv")
new_sent = []
for s in sent:
    s["sentiment_analysis"] = json.loads(
        s["sentiment_analysis"].replace("'", '"'))

    new_sent_row = copy.deepcopy(s)
    # new_sent_row["sentiment_analysis_negetive"] = new_sent_row["sentiment_analysis"]["negative"]
    # new_sent_row["sentiment_analysis_neutral"] = new_sent_row["sentiment_analysis"]["neutral"]
    # new_sent_row["sentiment_analysis_postive"] = new_sent_row["sentiment_analysis"]["positive"]

    new_sent_row["sentiment_analysis"] = find_max(
        new_sent_row["sentiment_analysis"])

    # del new_sent_row["sentiment_analysis"]
    new_sent.append(new_sent_row)


# print(new_sent)
write_to_csv(new_sent, "caption_sentiment_anlaysis_corrected.csv")
